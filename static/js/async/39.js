"use strict";
(self["webpackChunkmoebytes_site"] = self["webpackChunkmoebytes_site"] || []).push([["39"], {
3274(__unused_rspack___webpack_module__, __webpack_exports__, __webpack_require__) {
// ESM COMPAT FLAG
...
const Header = {
    ...
    get: (buf, off) => {
        return {
            // Group-ID
            chunkID: new lib.StringType(4, 'latin1').get(buf, off),
            // Size
            chunkSize: lib.UINT32_LE.get(buf, off + 4)
        };
    }
};
/**
...
class ListInfoTagValue {
    ...
    get(buf, off) {
        return new lib.StringType(this.tagHeader.chunkSize, 'ascii').get(buf, off);
    }
}
...
class Format {
    ...
    get(buf, off) {
        return {
            wFormatTag: lib.UINT16_LE.get(buf, off),
            nChannels: lib.UINT16_LE.get(buf, off + 2),
            nSamplesPerSec: lib.UINT32_LE.get(buf, off + 4),
            nAvgBytesPerSec: lib.UINT32_LE.get(buf, off + 8),
            nBlockAlign: lib.UINT16_LE.get(buf, off + 12),
            wBitsPerSample: lib.UINT16_LE.get(buf, off + 14)
        };
    }
}
/**
...
class FactChunk {
    ...
    get(buf, off) {
        return {
            dwSampleLength: lib.UINT32_LE.get(buf, off)
        };
    }
}
...
const BroadcastAudioExtensionChunk = {
    ...
    get: (uint8array, off) => {
        return {
            description: (0,Util/* .stripNulls */.F6)(new lib.StringType(256, 'ascii').get(uint8array, off)).trim(),
            originator: (0,Util/* .stripNulls */.F6)(new lib.StringType(32, 'ascii').get(uint8array, off + 256)).trim(),
            originatorReference: (0,Util/* .stripNulls */.F6)(new lib.StringType(32, 'ascii').get(uint8array, off + 288)).trim(),
            originationDate: (0,Util/* .stripNulls */.F6)(new lib.StringType(10, 'ascii').get(uint8array, off + 320)).trim(),
            originationTime: (0,Util/* .stripNulls */.F6)(new lib.StringType(8, 'ascii').get(uint8array, off + 330)).trim(),
            timeReferenceLow: lib.UINT32_LE.get(uint8array, off + 338),
            timeReferenceHigh: lib.UINT32_LE.get(uint8array, off + 342),
            version: lib.UINT16_LE.get(uint8array, off + 346),
            umid: new lib.Uint8ArrayType(64).get(uint8array, off + 348),
            loudnessValue: lib.UINT16_LE.get(uint8array, off + 412),
            maxTruePeakLevel: lib.UINT16_LE.get(uint8array, off + 414),
            maxMomentaryLoudness: lib.UINT16_LE.get(uint8array, off + 416),
            maxShortTermLoudness: lib.UINT16_LE.get(uint8array, off + 418)
        };
    }
};
...
const debug = browser('music-metadata:parser:RIFF');
/**
...
class WaveParser extends BasicParser/* .BasicParser */.s {
    ...
    async parseRiffChunk(chunkSize) {
        ...
        switch (type) {
            case 'WAVE':
                return this.readWaveChunk(chunkSize - FourCC/* .FourCcToken.len */.e.len);
            default:
        ...
        while (remaining >= Header.len) {
            ...
            switch (header.chunkID) {
                ...
                case 'ID3 ': { // The way Mp3Tags stores ID3 meta-data
                    ...
                    const rst = core/* .fromBuffer */.vY(id3_data);
    ...
    async parseListTag(listHeader) {
        const listType = await this.tokenizer.readToken(new lib.StringType(4, 'latin1'));
