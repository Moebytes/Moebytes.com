"use strict";
(self["webpackChunkmoebytes_site"] = self["webpackChunkmoebytes_site"] || []).push([["269"], {
8036(__unused_rspack___webpack_module__, __webpack_exports__, __webpack_require__) {
// ESM COMPAT FLAG
...
const ChunkHeader64 = {
    ...
    get: (buf, off) => {
        return {
            // Group-ID
            chunkID: FourCC/* .FourCcToken.get */.e.get(buf, off),
            // Size
...
const debug = browser('music-metadata:parser:aiff');
...
class DsdiffParser extends BasicParser/* .BasicParser */.s {
    async parse() {
        ...
        switch (type) {
            case 'DSD':
                ...
                return this.readFmt8Chunks(header.chunkSize - BigInt(FourCC/* .FourCcToken.len */.e.len));
            default:
    ...
    async readData(header) {
        ...
        switch (header.chunkID.trim()) {
            ...
            case 'PROP': { // 3.2 PROPERTY CHUNK
                ...
                await this.handleSoundPropertyChunks(header.chunkSize - BigInt(FourCC/* .FourCcToken.len */.e.len));
                break;
            }
    ...
    async handleChannelChunks(remainingSize) {
        ...
        const channels = [];
        while (remainingSize >= FourCC/* .FourCcToken.len */.e.len) {
            const channelId = await this.tokenizer.readToken(FourCC/* .FourCcToken */.e);
            debug(`Channel[ID=${channelId}]`);
            channels.push(channelId);
            remainingSize -= BigInt(FourCC/* .FourCcToken.len */.e.len);
        }
